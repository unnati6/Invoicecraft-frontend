...
export function EmailOrderDialog({
  ...
  const [formData, setFormData] = React.useState({
    ...
    cc: '',
    bcc: '',
    subject: `Invoice  - ${documentData?.invoiceNumber || ''} from ${companyBranding?.name || 'Your Company'}`,
    body: `Dear ${customerData?.name || 'Customer'},\n\nPlease find attached invoice #${documentData?.invoiceNumber || ''} for your reference.\n\nYou can view, print, and download your order form from the link below:\n[Invoice Link Placeholder - You'll replace this with actual link]\n\nThank you for your business!\n\nRegards,\n${companyBranding?.name || 'Your Company Name'}`,
  ...
  React.useEffect(() => {
    ...
    if (documentData && customerData && companyBranding) {
      setFormData(prev => ({
        ...
        to: customerData.email || '',
        subject: `Invoice - ${documentData.invoiceNumber || ''} from ${companyBranding.name || 'Your Company'}`,
        body: `Dear ${customerData.name || 'Customer'},\n\nPlease find attached invoice #${documentData.invoiceNumber || ''} for your reference.\n\nYou can view, print, and download your Invoice from the link below:\n[Invoice Link Placeholder - You'll replace this with actual link]\n\nThank you for your business!\n\nRegards,\n${companyBranding.name || 'Your Company Name'}`,
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await fetch(`${axiosInstance.defaults.baseURL}/invoices/${formData.documentId}/send-email`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            toast({
                title: "Success",
                description: result.message || "Invoice email sent successfully!",
            });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[700px]">  
        <DialogHeader>
          <DialogTitle>Email Invoice{documentData?.invoiceNumber}</DialogTitle>
          <DialogDescription>
